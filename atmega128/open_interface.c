...
{
    return (oi_encoder_t *)malloc(sizeof(oi_encoder_t));
}
...
{
    oi_t * ret = (oi_t *) malloc(sizeof(oi_t));
    ret->left_encoder = newEncoder();
    ret->right_encoder = newEncoder();
    return ret;
}

void freeOISensorData(oi_t * sensor)
{
    freeEncoder(sensor->left_encoder);
    freeEncoder(sensor->right_encoder);
    free(sensor);
...
inline void oi_reset()
{
    oi_sendChar(OI_OPCODE_RESET);
}

inline void oi_safe_mode()
{
    oi_sendChar(OI_OPCODE_SAFE);
}

inline void oi_full_mode()
{
    oi_sendChar(OI_OPCODE_FULL);
}

inline void oi_passive_mode()
{
    oi_init();
}
...
void oi_seek_dock()
{
    ...
    oi_sendChar(OI_OPCODE_SEEK_DOCK);
    oi_end();
}

void oi_set_leds(uint8_t check_robot_led, uint8_t dock_led, uint8_t spot_led, uint8_t debris_led, uint8_t power_color,
